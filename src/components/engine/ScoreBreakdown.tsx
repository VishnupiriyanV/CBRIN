...
export const ScoreBreakdown: React.FC<ScoreBreakdownProps> = ({ signals, reason }) => {
  ...
  return (
    <div className="space-y-2">
      <p className="text-[10px] font-mono text-ink-body">{reason}</p>
      <div className="space-y-1.5">
        {entries.map(([key, value]) => (
          <div key={key} className="flex items-center gap-2">
            <span className="text-[10px] font-mono text-ink-mute w-32 shrink-0">{SIGNAL_LABELS[key] || key}</span>
            <div className="flex-1 h-1.5 rounded-sm bg-canvas-soft border border-hairline/60 overflow-hidden">
              <div
                className="h-full rounded-sm bg-accent-sunset/80"
                style={{ width: `${Math.max(0, Math.min(1, value)) * 100}%` }}
              />
            </div>
            <span className="text-[10px] font-mono text-ink-body font-semibold w-10 text-right">
